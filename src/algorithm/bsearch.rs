...
pub fn upper_bound<T: Clone + Ord + PartialEq>(a: &[T], value: T) -> usize {
    let n = a.len();
    let mut ub = 0;
    let mut len = n;
...
pub fn equal_range<T: Clone + Ord + PartialEq>(a: &[T], value: T) -> (usize, usize) {
    (lower_bound(a, value.clone()), upper_bound(a, value.clone()))
}
...
mod tests {
    ...
    fn test_upper_bound() {
        let a = vec![1, 2, 3, 5, 6];
        assert_eq!(upper_bound(&a, 2), 2);
        assert_eq!(upper_bound(&a, 0), 0);
        assert_eq!(upper_bound(&a, 4), 3);
        assert_eq!(upper_bound(&a, 7), 5);
    }
