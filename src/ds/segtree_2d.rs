    ...
    M::Element: Clone,
{
    ...
    pub fn new(monoid: M, w: usize, h: usize) -> Self {
        let w = w.next_power_of_two() * 2;
        let h = h.next_power_of_two() * 2;
        let data = vec![vec![monoid.id(); h]; w];
        Self { monoid, data, w, h }
    }
...
mod tests {
    ...
    fn test() {
        ...
        for _ in 0..100 {
            ...
            let ans = a[wr]
                .iter()
                .map(|a| a[hr.clone()].iter().cloned().fold_m(&m))
                .fold_m(&m);
